...
template<typename T> void tracevp(vector<T> &a) { for(auto &i:a) { trace2(i.ff, i.ss); } cout<<'\n'; }
	...
	vector<int> a(3);
	int aa[7] = {0, 1, 2, 0, 2, 1, 0};
	vector<vector<int>> q = {{0, 1, }}
lli solve(vector<int> aa){
	return min(7*aa[0]/3 + q[0][aa[0]%3], min(7*aa[1]/2+q[1][aa[1]%2], 7*aa[2]/2+q[2][aa[2]%2]));
}
int main(){

	cin>>a[0]>>a[1]>>a[2];
	lli ans = 0;
	vector<vector<int>> cum(8, 0);
	for(i = 6 ; i >= 0 ; i--){
		cum[i] = cum[i+1];
		cum[i][aa[i]] -= 1;
	}
	for(int i = 0 ; i <= 7 ; i++){
		lli aaa = 7+solve(a-cum[i]);
		ans = max(ans, aaa);
	}
	cout<<ans;
