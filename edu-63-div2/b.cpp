...
int main(){
	...
	string s;
	cin>>s;
		priority_queue < ll, std::vector<ll>, std::greater<ll> > m,mm;	// mnheap.push(), mnheap.top(), mnheap.pop()
	for(int i = 0 ; i < n ; i++){
		if(s[i] == '8'){
			mm.push(i);
		}
	...
	if(mm.empty()) mm.push(INT_MAX);
	
	if(m.empty()) m.push(INT_MAX);

	if(s[min(m.top(), mm.top())] == '8') cout<<"YES";
	else cout<<"NO";
